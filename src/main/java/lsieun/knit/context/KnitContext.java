...
public class KnitContext {
    ...
    private final Map<KnittingPassState, InstructionBinding> passToBinding = new HashMap<>();

    // 2. 通过指令找所有相关的物理行程 (Instruction -> List<Pass>)
    private final Map<KnitInstruction, List<KnittingPassState>> instructionToPasses = new HashMap<>();

    /**
     * 建立绑定关系
     */
    public void bind(KnittingPassState pass, InstructionBinding binding) {
     ...
     * 查询：通过行程获取其身世
     */
    public Optional<InstructionBinding> getBinding(KnittingPassState pass) {
     ...
     * 查询：通过指令获取其产生的所有行程
     */
    public List<KnittingPassState> getPassesByInstruction(KnitInstruction ins) {
